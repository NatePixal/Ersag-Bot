...
async function handleTelegramUpdate(req, res) {
    ...
    if (tgId === env.ADMIN_TELEGRAM_ID) {
        if (message.text === '/admin') {
            ...
            return await sendMessage(botToken, tgId, adminText);
        }
    }
    ...
    if (leaderData.account_status === 'active' && leaderData.tokens > 0) {
        // Now you can talk to the AI if you have tokens!
        await sendMessage(botToken, tgId, "Salom! Men sizning Ersağ yordamchingizman. Savolingizni bering.", keyboard);
    } else {
